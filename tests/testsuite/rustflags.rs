...
fn build_rustflags_recompile() {
    ...
    let config = r#"
        ...
        rustflags = ["-Z", "bogus"]
        "#;
    let config_file = paths::root().join("foo/.corgi/config");
...
fn build_rustflags_with_home_config() {
    ...
    let home = paths::home();
    let home_config = home.join(".corgi");
...
fn remap_path_prefix_works() {
    ...
    p.cargo("run")
        ...
        .with_stdout("/foo/home/.corgi/registry/src/[..]/bar-0.1.0/src/lib.rs")
        .run();
}
...
fn host_config_rustflags_with_target() {
    ...
    let p = project()
        ...
        .file(".corgi/config.toml", "target-applies-to-host = false")
        .build();
