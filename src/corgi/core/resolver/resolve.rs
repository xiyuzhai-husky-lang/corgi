...
pub struct Resolve {
    ...
    /// `corgi::core::resolver::encode` for more.
...
impl Default for ResolveVersion {
    ...
    /// those generated by `corgi update` (update everything) or building after
    /// a `corgi new` (where no lock file previously existed). This is also used
