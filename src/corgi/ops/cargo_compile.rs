...
pub struct CompileOptions {
    ...
    /// defined in the corgi metadata for a crate.
...
pub fn create_bcx<'a, 'cfg>(
    ...
    // The ordering here affects some error messages coming out of corgi, so
        ...
        if unit.mode.is_doc() || unit.mode.is_doc_test() {
            ...
            if rustdoc_document_private_items || unit.target.is_bin() {
                ...
                if unit.target.is_bin() {
                    ...
                    // sometimes and ignore them at other times. But corgi consistently passes
    ...
    if honor_rust_version {
        ...
        for unit in unit_graph.keys() {
            ...
            let guidance = if ws.is_ephemeral() {
                if ws.ignore_lock() {
                    "Try re-running corgi install with `--locked`".to_string()
                } else {
                    String::new()
                }
...
impl CompileFilter {
    ...
    /// function only recognizes test targets, which means corgi might compile
    ...
    /// Selects targets for "corgi run". for logic to select targets for other
    ...
    let new_unit = |units: &mut HashSet<Unit>,
        ...
        for kind in explicit_kinds.iter() {
            let unit_for = if initial_target_mode.is_any_test() {
                ...
                //
                // Forcing the lib to be compiled three times during `corgi
...
/// and their code remains unchecked despite corgi not returning any errors
...
///
/// This is primarily used by `corgi rustc --crate-type`.
