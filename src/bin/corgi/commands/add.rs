...
pub fn cli() -> Command {
    clap::Command::new("add")
        ...
        .override_usage(
            "\
    corgi add [OPTIONS] <DEP>[@<VERSION>] ...
    corgi add [OPTIONS] --path <PATH> ...
    corgi add [OPTIONS] --git <URL> ..."
        )
        .after_help("Run `corgi help add` for more detailed information.\n")
...
- `<name>`, like `corgi add serde` (latest version will be used)
- `<name>@<version-req>`, like `corgi add serde@1` or `corgi add serde@=1.0.38`"
            )
                .group("selected"),
...
Without any other information, corgi will use latest commit on the main branch.")
                .group("selected"),
            clap::Arg::new("branch")
                .long("branch")
                .takes_value(true)
