...
pub fn cli() -> Command {
    subcommand("tree")
        ...
        .arg(
            // Backwards compatibility with old corgi-tree.
            flag("version", "Print version info and exit")
                .short('V')
                .hide(true),
        )
        .after_help("Run `corgi help tree` for more detailed information.\n")
}
...
pub fn exec(config: &mut Config, args: &ArgMatches) -> CliResult {
    ...
    if args.flag("all") {
        config.shell().warn(
            "The `corgi tree` --all flag has been changed to --no-dedupe, \
    ...
    if args.is_present_with_zero_values("invert") {
        match &packages {
            Packages::Packages(ps) => {
                // Backwards compatibility with old syntax of `corgi tree -i -p foo`.
