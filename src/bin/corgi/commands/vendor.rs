...
pub fn cli() -> Command {
    subcommand("vendor")
        ...
        .arg(flag(
            ...
            "Respect `[source]` config in `.corgi/config`",
        ))
        .arg(flag(
            "versioned-dirs",
        ...
        .after_help("Run `corgi help vendor` for more detailed information.\n")
}
...
pub fn exec(config: &mut Config, args: &ArgMatches) -> CliResult {
    ...
    // When we moved `corgi vendor` into Cargo itself we didn't stabilize a few
    ...
    if let Some(flag) = crates_io_cargo_vendor_flag {
        return Err(anyhow::format_err!(
            "\
the crates.io `corgi vendor` command has now been merged into Cargo itself
and does not support the flag `{}` currently; to continue using the flag you
can execute `corgi-vendor vendor ...`, and if you would like to see this flag
