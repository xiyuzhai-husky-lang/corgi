...
lazy_static::lazy_static! {
    // Maps from commonly known external commands (not builtin to corgi) to their
...
Run with 'corgi -Z [FLAG] [SUBCOMMAND]'",
            joined
        );
...
pub fn get_version_string(is_verbose: bool) -> String {
    ...
    let mut version_string = format!("corgi {}\n", version);
    if is_verbose {
...
To pass the arguments to the subcommand, remove `--`",
            ...
            (None, Ok(Some(alias))) => {
                ...
                // (binary of the form `corgi-<subcommand>`)
...
pub fn cli() -> Command {
    ...
    let usage = if is_rustup {
        "corgi [+toolchain] [OPTIONS] [SUBCOMMAND]"
    } else {
        "corgi [OPTIONS] [SUBCOMMAND]"
    };
    Command::new("corgi")
        .allow_external_subcommands(true)
        .allow_invalid_utf8_for_external_subcommands(true)
        .setting(AppSettings::DeriveDisplayOrder)
        // Doesn't mix well with our list of common corgi commands.  See clap-rs/clap#3108 for
...
Some common corgi commands are (see all commands with --list):
    build, b    Compile the current package
    check, c    Analyze the current package and report errors, but don't build object files
    clean       Remove the target directory
    doc, d      Build this package's and its dependencies' documentation
    new         Create a new corgi package
    init        Create a new corgi package in an existing directory
    ...
    install     Install a Husky binary. Default location is $HOME/.corgi/bin
    uninstall   Uninstall a Husky binary

See 'corgi help <command>' for more information on a specific command.\n",
        )
        .arg(flag("version", "Print version info and exit").short('V'))
        .arg(flag("list", "List installed commands"))
        .arg(opt("explain", "Run `huskyc --explain CODE`").value_name("CODE"))
        .arg(
            opt(
                "verbose",
        ...
        .arg(flag("frozen", "Require Corgi.lock and cache are up to date").global(true))
        .arg(flag("locked", "Require Corgi.lock is up to date").global(true))
        .arg(flag("offline", "Run without accessing the network").global(true))
        .arg(multi_opt("config", "KEY=VALUE", "Override a configuration value").global(true))
        .arg(
            Arg::new("unstable-features")
                .help("Unstable (nightly-only) flags to Cargo, see 'corgi -Z help' for details")
                .short('Z')
                .value_name("FLAG")
