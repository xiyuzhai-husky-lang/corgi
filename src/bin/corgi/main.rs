...
fn list_commands(config: &Config) -> BTreeMap<String, CommandInfo> {
    ...
    commands.insert(
        "help".to_string(),
        CommandInfo::BuiltIn {
            about: Some("Displays help for a corgi subcommand".to_string()),
        },
    );
...
fn find_external_subcommand(config: &Config, cmd: &str) -> Option<PathBuf> {
    let command_exe = format!("corgi-{}{}", cmd, env::consts::EXE_SUFFIX);
        ...
        None => {
            let err = if cmd.starts_with('+') {
                anyhow::format_err!(
                    ...
                    Did you mean to invoke `corgi` through `rustup` instead?",
                    cmd
                )
            } else {
                ...
                anyhow::format_err!(
                    "no such subcommand: `{}`{}\n\n\t\
                    View all installed commands with `corgi --list`",
                    cmd,
                    did_you_mean
                )
            };
